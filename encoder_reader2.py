...
def data_deal_s(data, last_s, mmpp):
    dis_mm = []
    s = []
    v = 0
    if data is not None:
...
class uart_flasher(threading.Thread):
    def __init__(self, uart, options) -> None:
        ...
        self.vs = deque([0], maxlen=options["MAXLEN"])
        self.ds = deque([0], maxlen=100*options["MAXLEN"])
        self.frames_d = deque([0], maxlen=100*options["MAXLEN"])
        self.frames_v = deque([0], maxlen=options["MAXLEN"])
    ...
    def run(self):
        frame =  0
        last_s = 0
        mmpp = calc_mmpp(options["PPR"])
        while uart.isOpen():
            if uart.in_waiting:
                frame += 1
                data = data_finder(self.uart)
                dis_mm, t_dis, v_mmps = data_deal_s(data, last_s, mmpp)
